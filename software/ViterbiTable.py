...
class ViterbiTable:
	...
	def _traceLabel(self, tracingUser, lastEmotionIndex, emotionLabels):
		for i in range(len(self._convo) - 1, -1, -1):
	...
	def _selectMaximizingRecvP(self, ecc, lastRecvIndex, emotion):
		psForEmotionRecv = [ecc.pGivenReceived(emotion, prevE) for prevE in self._emotions]
	...
	def initialize(self, ecc, emc):
		...
		for i in range(0, len(self._emotions)):
			...
			self._table[0].addRowToCell(pEmotion, -1)
		...
		for i in range(0, len(self._emotions)):
			...
			pEmotion = emc.test(inst, self._emotions[i])
			#pEmotionSent = ecc.pGivenSent(self._emotions[i], "START")
			#pEmotionRecv = self._selectMaximizingRecvP(ecc, user1msgIndex, self._emotions[i])
			self._table[1].addRowToCell(pEmotion, -1)
	...
	def iterate(self, user1msgIndex,user2msgIndex, ecc, emc):
		for i in range(2, len(self._convo)):
			newCell = ViterbiCell()
			msg = instance(None, self._convo[i].content())
	...
	def sequence(self):
		...
		emotionIndex = self._table[-1].selectMostLikely()
		...
		for i in range(len(self._convo) - 1, -1, -1):
			if self._convo[i].user() != tracingUser:
				emotionIndex = self._table[i].selectMostLikely()
